...
```
to produce the recontructed graph of Manhattan roads. The graph will be saved as a `.pkl` file in the `graphs/` folder. The script accepts the following flags:
...
To produce the visualization of the graphs as maps of Manhattan, run
```
cd mapping
python make_maps.py
```
which will create a `.html` interactive map in the `maps/` folder for each graph in the `graphs/` folder.
...
```
python generate_sequences_for_map.py
```
